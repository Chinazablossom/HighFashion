...
class HttpHandler {
  ...
  static Future<Map<String, dynamic>> postRequest(String endpoint,
    ...
    final response = await https.post(
        Uri.parse("$_baseUrl/$endpoint"), headers: {
      "Content-type": "application/json"
    }, body: json.encode(data)
    );

    if(response.statusCode >= 400){
      Get.snackbar("Failed to add item","An error occurred when trying to add item");
    }
  ...
  static Future<Map<String, dynamic>> deleteRequest(String endpoint,
    ...
    if(response.statusCode >= 400){
      Get.snackbar("Failed to delete item","An error occurred when trying to delete item");
    }
